...
local function HandleBullets()
	...
	for id, bullet in pairs( LVS._ActiveBullets ) do -- loop through bullet table
		...
		bullet.Dir = bullet.Dir + (Vector(0,0,  -TimeAlive / 10e9/4  ):GetNormalized())
		
		local dir = bullet.Dir

		local pos = (dir * TimeAlive * bullet.Velocity / 2) 
		...
		debugoverlay.Line( Is2ndTickAlive and start + pos - dir or start, start + pos + dir * bullet.Velocity * FT, 10, Color( 255, 255, 255 ), true )
		...
		if trace.Hit then
			...
			if SERVER then
				...
				if bullet.SplashDamage and bullet.SplashDamageRadius then
					...
					util.BlastDamage( bullet.Entity, bullet.Attacker, EndPos, bullet.SplashDamageRadius, bullet.SplashDamage)
...
if SERVER then
	...
	function LVS:FireBullet( data )
		...
		if not IsCannon[bullet.TracerName] then
			data.Entity:FireBullets(bullet)
	...
	net.Receive( "lvs_fire_bullet", function( length )
		...
		effectdata:SetAngles( bullet.Dir:Angle() )
